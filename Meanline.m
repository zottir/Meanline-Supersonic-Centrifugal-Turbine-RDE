...
% Trailing Edges
t_TE_s = 1.8*10^-3; % 1.8 mm
t_TE_r = 1.8*10^-3; % 1.8 mm
t_LE_min = 1.8*10^-3; % 1.8 mm
...
iTref = 1;
err = 2*tolTref;
while (iTref < NmaxTref) && (err > tolTref)
    ...
    while (ip1t < Nmaxp1t) && (err > tolp1t)
        ...
        q_m_stator = c_merid_stator/rad_solidity_stator;
        conc_s = 1;
    ...
    while (ip3t < Nmaxp3t) && (err > tolp3t)
        ...
        q_m_rotor = c_merid_rotor/rad_solidity_rotor;
        conc_r = 1;
...
% Info Machine
fprintf('With phi = %f and lambda = %f :\n',phi,lambda);
fprintf('Machine Efficiency = %f\n',efficiency_tt);
fprintf('Machine Power = %f [kW]\n',Power/1000);
